...
status = cycle( ["Jan Ken Pon", "Hanetsuki", "Keidoro", "Beigoma", "Menko",
"Karuta", "Ayatori", "Daruma Otoshi"
 ])
...
async def on_message(message):
    ...
    if msg.startswith('hello'):
      ...
      ch_name = a[1]
      output = a[2]
      chnl = client.get_channel( channel_id_1 )
      if( ch_name == 'general' ):
        chnl = client.get_channel( channel_id_2 )
      elif ( ch_name == 'amano' ):
        chnl = client.get_channel( channel_id_3 )
      elif ( ch_name == 'music' ):
        chnl = client.get_channel( channel_id_4 )
      elif ( ch_name == 'confess' ):
        chnl = client.get_channel( channel_id_5 )
      elif ( ch_name == 'vc' ):
        chnl = client.get_channel( channel_id_6 )
      elif ( ch_name == 'anime' ):
        chnl = client.get_channel( channel_id_7 )
      ...
      deleted = await message.delete()
       ...
       embed.set_author( name = '{}'.format(usr.name), icon_url = '{}'.format(usr.avatar_url))
       embed.set_thumbnail( url = '{}'.format(usr.avatar_url))
       embed.add_field( name= 'About {}'.format(usr.display_name), value='{}'.format(usr.avatar), inline=False )
      ...
      embed.set_author( name = '__HELP__', icon_url = 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/google/313/scroll_1f4dc.png') # Scroll emoji 
      embed.set_thumbnail( url = 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/scroll_1f4dc.png')
      embed.add_field( name = '__Commands__', value = ":wave: **Hello** : responds with Hello\n**~profile** : diplays profile\n**~list** : diplays list of Encouragements\n**~add <Message>** : new Encouragement\n**~del <index>** : delete Encouragement at index\n**responding true** : turns response to messages on\n**responding <anything else>** : turns off response to messages\n**~write <channel> <Message>** : writes Message in channel.\n**~clear <integer n>** : deletes n messages.", inline = False )
...
async def on_member_join(member):
  role = discord.utils.get( member.guild.roles , id=channel_id) # name = "otaku"
...
async def on_message(message):
    ...
    if msg.startswith('hello'):
      ...
      chnl = client.get_channel( channel_id_1 )
      if( ch_name == 'general' ):
        chnl = client.get_channel( chaqnnel_id_2 )
      elif ( ch_name == 'amano' ):
        chnl = client.get_channel( channel_id_3 )
      elif ( ch_name == 'music' ):
        chnl = client.get_channel( channel_id_4 )
